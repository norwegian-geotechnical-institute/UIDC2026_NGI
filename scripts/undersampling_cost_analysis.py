...
def analyze_undersampling_fraction(
    ...
    use_collapse_sections : bool
        If True and 'collapse_section' exists, use StratifiedGroupKFold to keep
        collapse sections together during CV (default: True)
    ...
    # Check if collapse_section column exists and set up CV strategy
    has_collapse_sections = use_collapse_sections and 'collapse_section' in X_train.columns
    
    if has_collapse_sections:
        # Create groups: use collapse_section for collapse samples, unique ID for non-collapse
        groups = X_train['collapse_section'].copy()
        non_collapse_mask = groups == 0
        groups.loc[non_collapse_mask] = -X_train.loc[non_collapse_mask].index
        
        # Remove collapse_section from features for training
        X_train_for_cv = X_train.drop(columns=['collapse_section'])
        X_test_for_training = X_test.drop(columns=['collapse_section']) if 'collapse_section' in X_test.columns else X_test
        ...
        cv_splitter = list(sgkf.split(X_train_for_cv, y_train, groups=groups))
    else:
        X_train_for_cv = X_train
        X_test_for_training = X_test
        # Perform standard stratified k-fold cross-validation on training set
        skf = StratifiedKFold(n_splits=cv_folds, shuffle=True, random_state=random_seed)
        cv_splitter = list(skf.split(X_train_for_cv, y_train))
    ...
    for train_idx, val_idx in cv_splitter:
        X_train_fold = X_train_for_cv.iloc[train_idx]
        X_val_fold = X_train_for_cv.iloc[val_idx]
    ...
    y_pred_test = train_predict(
        model_name=model_name,
        model_params=model_params,
        X_train=X_train_for_cv,
        X_test=X_test_for_training,
        y_train=y_train,
        y_test=y_test,
        undersample_level=undersample_level,
        oversample_level=oversample_level,
    ...
    # Calculate test set accuracy metrics from confusion matrix counts
    tn = test_cost_metrics["tn_count"]
    fp = test_cost_metrics["fp_count"]
    fn = test_cost_metrics["fn_count"]
    tp = test_cost_metrics["tp_count"]
    total = len(y_test)
    
    test_accuracy = (tp + tn) / total if total > 0 else 0
...
def plot_cost_vs_undersampling(
    ...
    include_test : bool
        If True, include test set costs on the plot (default: True)
    """
    ...
    test_avg_costs = [r["test_cost_average"] for r in results]
    ...
    fig.suptitle("Undersampling Fraction vs Prediction Cost Analysis (5-Fold CV)", fontsize=12, fontweight="bold")
    
    # Plot 1: Average Cost with error bars
    ax1 = axes[0]
    ax1.errorbar(fractions, avg_costs, yerr=avg_costs_std, marker="o", linewidth=1.5, 
                 markersize=4, color="#2E86AB", capsize=3, capthick=1.5, alpha=0.8, label="CV (Training)")
    
    if include_test:
        ax1.plot(fractions, test_avg_costs, marker="s", linewidth=1.5, 
                 markersize=4, color="#FF6B6B", alpha=0.8, linestyle="--", label="Test Set")
    
    ax1.set_xlabel("Undersampling Fraction, Majority:Minority (x:1)", fontsize=9)
    ax1.set_ylabel(f"Average Cost per Prediction ({time_unit})", fontsize=9)
    ax1.set_title(f"Average Cost vs Undersampling Fraction", fontsize=10, fontweight="bold")
    ...
    ax1.scatter([fractions[min_idx]], [avg_costs[min_idx]], 
                color="red", s=60, zorder=5, label=f"Min CV: {fractions[min_idx]:.1f}")
    
    # Mark minimum for test if included
    if include_test:
        min_test_idx = np.argmin(test_avg_costs)
        ax1.scatter([fractions[min_test_idx]], [test_avg_costs[min_test_idx]], 
                    color="darkred", s=60, zorder=5, marker="^", label=f"Min Test: {fractions[min_test_idx]:.1f}")
    ...
    ax2.stackplot(fractions, tn_avg_costs, fp_avg_costs, fn_avg_costs, tp_avg_costs,
                  ...
                  alpha=0.8)
    ax2.set_xlabel("Undersampling Fraction, Majority:Minority (x:1)", fontsize=9)
    ax2.set_ylabel(f"Average Cost Components per Prediction ({time_unit})", fontsize=9)
    ax2.set_title(f"Cost Component Breakdown", fontsize=10, fontweight="bold")
    ...
    # ============================================================================
    fig_errors, ax_left = plt.subplots(1, 1, figsize=(7, 4))
    ...
    line_fp = ax_left.plot(fractions, fp_counts, marker="s", linewidth=1.5, markersize=4, 
                            label="False Collapse", color=fp_color)
    line_fn = ax_left.plot(fractions, fn_counts, marker="^", linewidth=1.5, markersize=4,
                            label="False Regular", color=fn_color)
    line_tp = ax_left.plot(fractions, tp_counts, marker="d", linewidth=1.5, markersize=4,
                            label="True Collapse", color=tp_color)
    
    # Plot TN on right axis
    line_tn = ax_right.plot(fractions, tn_counts, marker="o", linewidth=1.5, markersize=4, 
                             label="True Regular", color=tn_color)
    
    # Configure left axis
    ax_left.set_xlabel("Undersampling Fraction, Majority:Minority (x:1)", fontsize=9)
    ax_left.set_ylabel("Count (FC, FR, TC)", fontsize=9, color="black")
    ax_left.tick_params(axis='both', labelcolor="black", labelsize=7)
    ...
    ax_right.set_ylabel("Count (TR)", fontsize=9, color=tn_color)
    ax_right.tick_params(axis='both', labelcolor=tn_color, labelsize=7)
    ...
    # Title
    ax_left.set_title("Confusion Matrix Counts vs Undersampling Fraction", fontsize=10, fontweight="bold")
    
    # Combine legends from both axes
    lines = line_fp + line_fn + line_tp + line_tn
    labels = [l.get_label() for l in lines]
    ax_left.legend(lines, labels, fontsize=7, loc="center right")
...
def plot_accuracy_metrics(
    ...
    -----------
    use_test_data : bool
        If True, plot test set metrics. If False, plot CV metrics (default: False)
    """
    
    fractions = [r["undersample_fraction"] for r in results]
    
    if use_test_data:
        # Use test set metrics (no error bars)
        accuracies = [r["test_accuracy"] for r in results]
        balanced_accuracies = [r["test_balanced_accuracy"] for r in results]
        sensitivities = [r["test_sensitivity"] for r in results]
        precisions = [r["test_precision"] for r in results]
        f1_scores = [r["test_f1_score"] for r in results]
        has_error_bars = False
        title_suffix = "Test Set"
    else:
        # Use CV metrics (with error bars)
    ...
    # Plot metrics (with or without error bars)
    if has_error_bars:
        ax.errorbar(fractions, accuracies, yerr=accuracies_std, linewidth=1.5, 
                label="Accuracy", color="#2E86AB", alpha=0.7, capsize=2)
        ax.errorbar(fractions, balanced_accuracies, yerr=balanced_accuracies_std, linewidth=1.5,
                label="Balanced Accuracy", color="#A23B72", alpha=0.7, capsize=2)
        ax.errorbar(fractions, sensitivities, yerr=sensitivities_std, linewidth=1.5,
                label="Recall", color="#F18F01", alpha=0.7, capsize=2)
        ax.errorbar(fractions, precisions, yerr=precisions_std, linewidth=1.5,
                label="Precision", color="#C73E1D", alpha=0.7, capsize=2)
        ax.errorbar(fractions, f1_scores, yerr=f1_scores_std, linewidth=1.5,
                label="F1-Score", color="#6A994E", alpha=0.7, capsize=2)
    else:
        ax.plot(fractions, accuracies, marker="o", linewidth=1.5, markersize=4,
                label="Accuracy", color="#2E86AB", alpha=0.7)
        ax.plot(fractions, balanced_accuracies, marker="o", linewidth=1.5, markersize=4,
                label="Balanced Accuracy", color="#A23B72", alpha=0.7)
        ax.plot(fractions, sensitivities, marker="o", linewidth=1.5, markersize=4,
                label="Recall", color="#F18F01", alpha=0.7)
        ax.plot(fractions, precisions, marker="o", linewidth=1.5, markersize=4,
                label="Precision", color="#C73E1D", alpha=0.7)
        ax.plot(fractions, f1_scores, marker="o", linewidth=1.5, markersize=4,
                label="F1-Score", color="#6A994E", alpha=0.7)
    ...
    ax.scatter([fractions[max_acc_idx]], [accuracies[max_acc_idx]], 
              color="#2E86AB", s=60, zorder=5, marker="*", edgecolors="black", linewidths=1)
    ax.scatter([fractions[max_bal_acc_idx]], [balanced_accuracies[max_bal_acc_idx]], 
              color="#A23B72", s=60, zorder=5, marker="*", edgecolors="black", linewidths=1)
    ax.scatter([fractions[max_sens_idx]], [sensitivities[max_sens_idx]], 
              color="#F18F01", s=60, zorder=5, marker="*", edgecolors="black", linewidths=1)
    ax.scatter([fractions[max_prec_idx]], [precisions[max_prec_idx]], 
              color="#C73E1D", s=60, zorder=5, marker="*", edgecolors="black", linewidths=1)
    ax.scatter([fractions[max_f1_idx]], [f1_scores[max_f1_idx]], 
              color="#6A994E", s=60, zorder=5, marker="*", edgecolors="black", linewidths=1)
    
    # Add vertical line at minimum cost
    ax.axvline(x=min_cost_fraction, color="red", linestyle="--", alpha=0.5, linewidth=1,
               label="Minimum cost")
    ...
    ax.set_xlabel("Undersampling Fraction, Majority:Minority (x:1)", fontsize=9)
    ax.set_ylabel("Metric Value", fontsize=9)
    ax.set_title(f"Accuracy Metrics vs Undersampling Fraction ({title_suffix})", fontsize=10, fontweight="bold")
    ax.tick_params(axis='both', labelsize=8)
    ax.set_ylim(0, 1.05)
    ax.legend(handles=handles, fontsize=7, loc="lower center", ncol=7, bbox_to_anchor=(0.5, -0.15))
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    
    # Save plot
    filename_suffix = "test" if use_test_data else "cv"
    accuracy_plot_path = results_dir / f"accuracy_metrics_vs_undersampling_{filename_suffix}.png"
...
def save_confusion_matrices(
    ...
    for idx in indices:
        ...
        # Show raw counts with TR/TC/FR/FC labels
...
def main(cfg: DictConfig) -> None:
    ...
    main_plot_path, error_plot_path = plot_cost_vs_undersampling(results, results_dir, cost_matrix, colors, time_unit="hours", include_test=True)
    console.print(f"  Saved: {main_plot_path}")
    console.print(f"  Saved: {error_plot_path}")
    
    accuracy_plot_cv_path = plot_accuracy_metrics(results, results_dir, use_test_data=False)
    console.print(f"  Saved: {accuracy_plot_cv_path}")
    
    accuracy_plot_test_path = plot_accuracy_metrics(results, results_dir, use_test_data=True)
    console.print(f"  Saved: {accuracy_plot_test_path}")
